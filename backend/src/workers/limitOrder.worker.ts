...
// In-memory cache — refreshed every 30s and whenever an order is filled/expired
...
const fillOrder = async (order: DbPendingOrder, ltp: number) => {
...
export const startLimitOrderWorker = () => {
  ...
  // Refresh cache and expire stale orders every 30s
  setInterval(async () => {
